...
- **잔여 콘솔 게이트화**: gate 모드에서 ① **재실행(전부-완료) 마일스톤 세션**이 무확인 시작 대신 confirm 게이트(게이트 전 대시보드 자동 보장 — M35 라이브 발견 갭 해소) ② **push 실패 재시도**도 confirm 게이트 — 무터미널 사이클이 에러 경로까지 완결. legacy(API 어댑터)·console·`--yes` 무변경
- **라이브가 잡은 수정 2건**: idle 상태 게이트 카드 미표시(표시 조건의 `run_active` 의존 제거 + WAITING 배지), legacy+gate 설정 조합의 게이트 오발동 가드
- **라이브 검증**: 라이브 제목·행 펼침(R0 FAIL 피드백→R1 재투입 보고)·재실행 confirm→릴리즈 text 게이트 무터미널 완주 — 전 항목 시각 확인
- **테스트**: 374개 (372 → 374, +2개)

### [v0.32.0]
- **통합 실행 — 대시보드 내장 기동 (M35, Phase 3c)**: **gate 모드 `porpoise` 한 번이면** 대시보드가 같은 프로세스의 백그라운드 스레드로 자동 기동 + 브라우저 오픈 — **터미널 1개**로 게이트 운영 전체 시작. conductor 종료 시 함께 닫힘. `[conductor] serve_dashboard`(미설정=gate 모드 자동, false=끔, true=console에서도)
- **공존 처리**: 이미 `porpoise dashboard`가 떠 있으면(포트 사용 중) 에러 없이 **기존 대시보드와 공존** — 안내 + 브라우저 오픈만. 자동 등록 덕에 현재 프로젝트가 기존 대시보드 셀렉터에도 즉시 등장. 기동 실패는 경고만(실행 계속)
- **무결합 불변**: 같은 프로세스여도 통신은 파일 매개(live.json·control/) 그대로
- **레지스트리 위생**: `/api/projects`(셀렉터)가 **실존 프로젝트만** 노출 — 삭제된 항목 숨김(읽기 필터, 파일 무수정). 단위 테스트의 레지스트리 오염 부작용도 수정
- **라이브 검증**: ① 터미널 1개 통합 실행(자동 기동→브라우저→게이트→완주) ② 공존(기존 대시보드 유지·독립 수명·게이트 전 사이클 완주) 모두 PASS
- **알려진 한계**: 재실행 시 전부-완료 상태의 마일스톤 생성 세션은 아직 게이트 없이 시작(M36 후보), 내장 포트 7878 고정
- **테스트**: 372개 (369 → 372, +3개)
